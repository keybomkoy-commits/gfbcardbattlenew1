...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Load persistent card templates from database (self-healing seed)
  try {
    const list = await dbLib.getCardTemplates();
    if (list && list.length > 0) {
      engine.CARD_TEMPLATES.splice(0, engine.CARD_TEMPLATES.length, ...list);
      console.log("Persistent card templates loaded successfully:", list.length);
    } else {
      // Seed templates if collection is brand new
      for (const tmpl of engine.CARD_TEMPLATES) {
        await dbLib.saveCardTemplate(tmpl.id, tmpl);
      }
      console.log("Firestore templates collection seeded with default templates.");
    }
  } catch (err) {
    console.warn("Non-blocking card templates load/seed warning:", err);
  }
...
function setupEventListeners() {
  ...
  document.getElementById('submit-login-btn').addEventListener('click', async (e) => {
    ...
    btn.disabled = true;
    btn.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i> กำลังตรวจสอบข้อมูล...';
  ...
  document.getElementById('submit-signup-btn').addEventListener('click', async (e) => {
    const btn = e.currentTarget;
    ...
    const originalHTML = btn.innerHTML;
    btn.disabled = true;
    btn.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i> กำลังสร้างสโมสร...';
  ...
  // Forfeit / Exit Match Button
  document.getElementById('forfeit-match-btn').addEventListener('click', (e) => {
    const btn = e.currentTarget;
    const mode = btn.dataset.mode || 'pvp';

    if (mode === 'bot') {
      showConfirmModal("คุณแน่ใจหรือไม่ว่าต้องการออกจากแมตช์การแข่งขันกับบอท? (การออกจะไม่ได้รับเงินและไม่มีค่าปรับ)", () => {
        cleanupMatchContext();
      ...
      showConfirmModal("คุณแน่ใจหรือไม่ว่าต้องการยอมแพ้และออกจากการแข่งขัน? การยอมแพ้จะเสียค่าปรับ 300 💰", async () => {
        ...
        if (activeRoomCode && activeRoomCode !== 'BOT_ROOM') {
          try {
            ...
            console.error("Failed to update forfeit status online:", err);
          }
        }
...
function createCardHTML(card) {
  ...
  // Sync image URL from latest templates so old inventory cards update automatically
  const tmpl = engine.CARD_TEMPLATES.find(t => t.id === card.templateId);
  const finalImageUrl = (tmpl && tmpl.imageUrl) ? tmpl.imageUrl : (card.imageUrl || '');

  if (finalImageUrl) {
    // Custom full-card graphic mode: hide all overlaid text/stats
    return `
      <div class="card-3d flipped">
        <div class="card-face card-back">
          <i class="fa-solid fa-circle-nodes"></i>
        </div>
        <div class="card-face card-front rarity-${card.rarity}" style="padding:0; overflow:hidden;">
          <img src="${finalImageUrl}" style="position:absolute; top:0; left:0; width:100%; height:100%; object-fit:cover; opacity:1.0; z-index:5; pointer-events:none; border-radius:12px;">
          
          ${isBanned ? `<div class="card-ban-badge" style="z-index: 10;"><i class="fa-solid fa-hand"></i><br>Banned<br>(${card.banMatches} Match)</div>` : ''}
          
          ${isYellow ? `
            <div class="card-yellows-badges" style="z-index: 10; position: absolute; bottom: 10px; right: 10px; display: flex; gap: 4px;">
  ...
  // Default procedural stats card mode: overlay rating, name, stats, and skills on card
  return `
    <div class="card-3d flipped">
      <div class="card-face card-back">
        <i class="fa-solid fa-circle-nodes"></i>
...
async function buyDynamicGachaPack(packId) {
  const loadingOverlay = document.getElementById('gacha-loading-overlay');
  try {
    const packs = await dbLib.getGachaPacks();
    ...
    // Bulletproof duplicate check: check templateId match OR fallback name match (for legacy player records)
    const isDuplicate = currentUser.inventory.some(c => 
      (c.templateId && newCard.templateId && c.templateId === newCard.templateId) ||
      (c.name && newCard.name && c.name.toLowerCase().trim() === newCard.name.toLowerCase().trim())
    );
    
    let refundCoins = 0;
    if (isDuplicate) {
      const refundRates = { common: 150, rare: 300, epic: 800, legendary: 2000 };
      refundCoins = refundRates[newCard.rarity] || 150;
      currentUser.coins += refundCoins;
    } else {
    ...
    // Wait slightly to give a realistic opening feel
    setTimeout(() => {
      if (loadingOverlay) loadingOverlay.classList.remove('active');
      openGachaRevealModal(newCard, isDuplicate, refundCoins);
    }, 1200);
...
function openGachaRevealModal(card, isDuplicate = false, refundCoins = 0) {
  ...
  // Customize Close Actions button based on duplicate status
  const closeBtn = document.getElementById('gacha-reveal-close-btn');
  if (closeBtn) {
    if (isDuplicate) {
      closeBtn.innerHTML = `<i class="fa-solid fa-coins"></i> การ์ดซ้ำ! แลกเปลี่ยนเป็น +${refundCoins} เหรียญ`;
      closeBtn.style.background = 'var(--accent-gold)';
      closeBtn.style.borderColor = 'var(--accent-gold)';
      closeBtn.style.color = 'var(--text-dark)';
    } else {
      closeBtn.innerHTML = '<i class="fa-solid fa-circle-check"></i> เพิ่มเข้าสู่สโมสร';
      closeBtn.style.background = 'var(--accent-neon-green)';
      closeBtn.style.borderColor = 'var(--accent-neon-green)';
      closeBtn.style.color = 'var(--text-dark)';
    }
  }

  // Reset status message
  const statusMsg = document.getElementById('gacha-reveal-status-msg');
  if (statusMsg) {
    ...
    card3D.addEventListener('click', () => {
      if (!card3D.classList.contains('flipped')) {
        ...
        if (statusMsg) {
          if (isDuplicate) {
            statusMsg.innerHTML = `
              <div style="color: var(--accent-gold); text-shadow: 0 0 10px rgba(250, 190, 0, 0.3); font-size: 1.05rem; background: rgba(5,8,15,0.85); padding: 10px 20px; border-radius: 8px; border: 1px solid rgba(250, 190, 0, 0.3); display: inline-flex; align-items: center; gap: 8px; font-weight: 700; animation: fadeIn 0.4s ease;">
                <i class="fa-solid fa-triangle-exclamation"></i> การ์ดผู้เล่นซ้ำ! แปลงเป็นเงินชดเชย +${refundCoins} เหรียญสโมสร 💰
              </div>
            `;
            showNotification(`การ์ดซ้ำ! ระบบได้ทำการแปลงเป็นเหรียญชดเชย +${refundCoins} เหรียญเรียบร้อยแล้ว`, "warning");
          } else {
            statusMsg.innerHTML = `
              <div style="color: var(--accent-neon-green); text-shadow: 0 0 10px rgba(0, 220, 80, 0.3); font-size: 1.05rem; background: rgba(5,8,15,0.85); padding: 10px 20px; border-radius: 8px; border: 1px solid rgba(0, 220, 80, 0.3); display: inline-flex; align-items: center; gap: 8px; font-weight: 700; animation: fadeIn 0.4s ease;">
                <i class="fa-solid fa-circle-check"></i> ได้รับผู้เล่นใหม่! เข้าร่วมสโมสรสำเร็จ 🏆
              </div>
            `;
            showNotification(`ได้รับผู้เล่นใหม่: ${card.name} เข้าร่วมสโมสรสำเร็จ!`, "success");
          }
...
function startMatchDocSubscription(roomCode) {
  ...
  matchUnsubscribe = dbLib.subscribeToMatch(roomCode, (match) => {
    ...
    if (match.status === 'waiting') {
      ...
      if (isHost) {
        // Auto-start match immediately (Quick Match & Custom match automatic kickoff)
        (async () => {
          try {
            const updates = {
              status: 'playing',
              logs: [
                ...match.logs,
                { min: 0, text: `จับคู่สำเร็จ! เริ่มต้นการแข่งขันทันที!`, type: 'whistle' }
              ]
            };
            await dbLib.updateMatchRoom(roomCode, updates);
          } catch (e) {
            console.error("Auto-start match error:", e);
          }
        })();
      } else {
        document.getElementById('lobby-desc-txt').textContent = `จับคู่สำเร็จ! กำลังเชื่อมต่อเข้าสู่สนามแข่ง...`;
        document.getElementById('start-match-btn').style.display = 'none';
      }
    } 
    else if (match.status === 'playing' || match.status === 'halftime') {
      ...
      if (isHost && !simulationInterval && match.status === 'playing') {
...
function renderMatchArenaUpdate(match) {
  // Update forfeit/exit button based on bot vs PvP match mode
  const forfeitBtn = document.getElementById('forfeit-match-btn');
  if (forfeitBtn) {
    if (isBotMatch) {
      forfeitBtn.dataset.mode = 'bot';
      forfeitBtn.innerHTML = '<i class="fa-solid fa-right-from-bracket"></i> ออกจากการแข่งขัน';
      forfeitBtn.style.background = 'rgba(255,255,255,0.08)';
      forfeitBtn.style.borderColor = 'rgba(255,255,255,0.2)';
      forfeitBtn.style.color = 'var(--text-light)';
    } else {
      forfeitBtn.dataset.mode = 'pvp';
      forfeitBtn.innerHTML = '<i class="fa-solid fa-flag"></i> ยอมแพ้และออกจากการแข่งขัน';
      forfeitBtn.style.background = 'rgba(220, 53, 69, 0.2)';
      forfeitBtn.style.borderColor = 'rgba(220, 53, 69, 0.4)';
      forfeitBtn.style.color = '#ff6b6b';
    }
  }
  ...
  if (match.logs.length > 0) {
    ...
    if (match.players.p1 && match.players.p1.inventory) {
      match.players.p1.inventory.forEach(c => {
        if (lastLog.text.includes(c.name)) p1Card = c;
      });
    }
    if (match.players.p2 && match.players.p2.inventory) {
...
function openHalftimeSubstitutionModal() {
  selectedHalfActiveId = null;
  selectedHalfBenchId = null;

  if (!currentMatchState || !currentMatchState.players) {
    console.warn("openHalftimeSubstitutionModal: Match state or players list is undefined.");
    return;
  }

  const userRole = (currentMatchState.players.p1 && currentMatchState.players.p1.uid === currentUser.uid) ? 'p1' : 'p2';
  const playerState = currentMatchState.players[userRole];
  if (!playerState || !playerState.squad || !playerState.squad.slots || !playerState.inventory) {
    console.warn("openHalftimeSubstitutionModal: Squad slots or inventory is missing for role:", userRole);
    return;
  }
  ...
  // Render starting actives as drop targets
  for (const key in playerState.squad.slots) {
    ...
    if (card) {
      const slotEl = document.createElement('div');
      slotEl.className = 'glass-panel halftime-slot-row';
      slotEl.style.padding = '10px 14px';
      slotEl.style.borderRadius = '10px';
      slotEl.style.border = '1px solid var(--glass-border)';
      slotEl.style.display = 'flex';
      slotEl.style.alignItems = 'center';
      slotEl.style.justifyContent = 'space-between';
      slotEl.style.transition = 'all 0.25s ease';
      slotEl.style.background = 'rgba(255, 255, 255, 0.02)';
      
      const posColors = { GK: '#e53935', DF: '#ffb300', MF: '#1e88e5', FW: '#43a047' };
      const posCol = posColors[card.position] || 'var(--text-muted)';
      
      slotEl.innerHTML = `
        <div style="display:flex; align-items:center; gap:10px;">
          <span style="background:${posCol}; color:white; font-size:0.75rem; font-weight:800; padding:2px 6px; border-radius:4px; min-width:30px; text-align:center;">${card.position}</span>
          <span style="font-weight:600; color:var(--text-light); font-size:0.85rem;">${card.name}</span>
        </div>
        <span style="background:rgba(255,213,79,0.15); color:var(--accent-gold); font-weight:800; font-size:0.8rem; padding:2px 6px; border-radius:4px;">OVR ${card.ovr}</span>
      `;
      
      // Set up dropzone events on starting slots
      slotEl.addEventListener('dragover', (e) => {
        e.preventDefault();
        slotEl.style.borderColor = 'var(--accent-neon-green)';
        slotEl.style.boxShadow = '0 0 10px var(--accent-neon-glow)';
        slotEl.style.background = 'rgba(0, 220, 80, 0.05)';
      });
      slotEl.addEventListener('dragleave', () => {
      ...
      slotEl.addEventListener('drop', async (e) => {
        ...
        try {
          const dragData = JSON.parse(e.dataTransfer.getData('text/plain'));
          const benchCardId = dragData.cardId;
          const benchCardPos = dragData.position;

          if (card.position !== benchCardPos) {
            showNotification(`ตำแหน่งไม่ตรงกัน! ต้องเป็นนักเตะตำแหน่ง ${card.position} เหมือนกันเท่านั้น`, "warning");
            return;
          }

          // Swap starting card (instId) with bench card (benchCardId)
          const slotKey = Object.keys(playerState.squad.slots).find(k => playerState.squad.slots[k] === instId);
          if (slotKey) {
            playerState.squad.slots[slotKey] = benchCardId;
            playerState.squad.bench = playerState.squad.bench.map(id => id === benchCardId ? instId : id);
          ...
          console.error("Halftime drop error:", err);
        }
      });
  ...
  // Render bench substitutes as draggable items
    ...
    if (card) {
      ...
      slotEl.innerHTML = `
        ...
        <div style="display:flex; align-items:center; gap:8px;">
          ${isBanned ? '<span style="color:#ef5350; font-size:0.7rem; font-weight:700;">[BANNED]</span>' : ''}
          <span style="background:rgba(255,213,79,0.15); color:var(--accent-gold); font-weight:800; font-size:0.8rem; padding:2px 6px; border-radius:4px;">OVR ${card.ovr}</span>
        </div>
      `;
  ...
  if (summaryModal) {
    ...
    if (keyEvents.length === 0) {
      ...
      keyEvents.forEach(evt => {
        ...
        if (evt.type === 'goal') {
          const teamName = (evt.side === 'p1') ? match.players.p1.username : (match.players.p2 ? match.players.p2.username : 'ทีมบอท');
          const scorer = evt.scorer || 'ผู้เล่น';
          const assist = evt.assist || 'ไม่มี';
          entry.innerHTML = `<span style="color:var(--accent-neon-green); font-weight:700;">${evt.min}'</span> <span>${icon} [${teamName}] <strong>${scorer}</strong> ยิงประตู (แอสซิสต์: ${assist})</span>`;
        } else {
...
async function renderAdminDashboard() {
  if (currentAdminTabValue === 'users') {
    ...
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px 0;"><i class="fa-solid fa-circle-notch fa-spin" style="color:var(--accent-purple); font-size: 1.2rem;"></i> กำลังโหลดข้อมูลผู้จัดการทีม...</td></tr>';
    try {
    ...
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px 0;"><i class="fa-solid fa-circle-notch fa-spin" style="color:var(--accent-gold); font-size: 1.2rem;"></i> กำลังโหลดข้อมูลตู้สุ่ม...</td></tr>';
    try {
    ...
    tbody.innerHTML = '<tr><td colspan="8" style="text-align:center; padding:20px 0;"><i class="fa-solid fa-circle-notch fa-spin" style="color:var(--accent-neon-green); font-size: 1.2rem;"></i> กำลังโหลดข้อมูลห้องแข่งขันปัจจุบัน...</td></tr>';
    try {
...
async function saveAdminCardEdits() {
  ...
  // Persist template online/offline database
  try {
    await dbLib.saveCardTemplate(newCard.id, newCard);
    showNotification("บันทึกแม่แบบการ์ดลงฐานข้อมูลเรียบร้อยแล้ว", "success");
  } catch (err) {
    console.error("Failed to save card template online:", err);
    showNotification("ไม่สามารถบันทึกแม่แบบการ์ดลงเซิร์ฟเวอร์ได้: " + err.message, "warning");
  }
...
function deleteAdminCardTemplate() {
  const idVal = document.getElementById('admin-card-modal-id').value;
  if (!idVal) return;
  showConfirmModal("ต้องการลบแม่แบบการ์ดนี้ออกจากฐานข้อมูลหลักหรือไม่?", async () => {
    const idx = engine.CARD_TEMPLATES.findIndex(c => c.id === idVal);
    if (idx !== -1) engine.CARD_TEMPLATES.splice(idx, 1);

    try {
      await dbLib.deleteCardTemplate(idVal);
      showNotification("ลบแม่แบบการ์ดเรียบร้อยแล้ว", "success");
    } catch (err) {
      console.error("Failed to delete card template online:", err);
      showNotification("ไม่สามารถลบแม่แบบการ์ดจากเซิร์ฟเวอร์ได้: " + err.message, "warning");
    }
