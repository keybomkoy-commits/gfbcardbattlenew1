...
async function buyDynamicGachaPack(packId) {
  ...
  try {
    ...
    const isDuplicate = currentUser.inventory.some(c => c.templateId === newCard.templateId);
  ...
  for (const key in playerState.squad.slots) {
    ...
    if (card) {
      ...
      slotEl.addEventListener('click', () => {
        ...
        selectedHalfBenchId = null;
        activeContainer.querySelectorAll('.halftime-slot-row').forEach(d => {
        ...
        document.getElementById('halftime-sub-status-msg').innerHTML = `เลือกตัวจริง <strong>${card.name} (${card.position})</strong> แล้ว. โปรดคลิกเลือกตัวสำรองตำแหน่ง <strong>${card.position}</strong> ทางขวาเพื่อเปลี่ยนตัว`;
        document.getElementById('halftime-sub-status-msg').style.color = 'var(--accent-neon-green)';
      });
  ...
  playerState.squad.bench.forEach(instId => {
    ...
    if (card) {
      ...
      slotEl.addEventListener('click', async () => {
        ...
        if (!selectedHalfActiveId) {
          showNotification("กรุณาเลือกนักเตะตัวจริงในสนาม (ฝั่งซ้าย) ที่ต้องการเปลี่ยนออกก่อน!", "warning");
          return;
        }
        ...
        if (selectedHalfActiveId && selectedHalfBenchId) {
          ...
          if (!activeCard || activeCard.position !== card.position) {
            showNotification(`ตำแหน่งไม่ตรงกัน! ต้องเป็นนักเตะตำแหน่ง ${activeCard ? activeCard.position : card.position} เหมือนกันเท่านั้น`, "warning");
            selectedHalfActiveId = null;
            selectedHalfBenchId = null;
            openHalftimeSubstitutionModal(); // Redraw to clear active borders
            return;
          }
