...
// Card Templates persistence (Firestore + Mock Local DB)
export async function getCardTemplates() {
  if (isOnline) {
    const colRef = firebaseSDK.collection(db, 'card_templates');
    const snap = await firebaseSDK.getDocs(colRef);
    const list = [];
    snap.forEach(d => list.push({ ...d.data(), id: d.id }));
    return list;
  } else {
    const list = localStorage.getItem('offline_card_templates');
    return list ? JSON.parse(list) : [];
  }
}

export async function saveCardTemplate(id, template) {
  if (isOnline) {
    await firebaseSDK.setDoc(firebaseSDK.doc(db, 'card_templates', id), template);
  } else {
    ...
    localStorage.setItem('offline_card_templates', JSON.stringify(list));
  }
}

export async function deleteCardTemplate(id) {
  if (isOnline) {
    await firebaseSDK.deleteDoc(firebaseSDK.doc(db, 'card_templates', id));
  } else {
    const list = await getCardTemplates();
    const filtered = list.filter(c => c.id !== id);
    localStorage.setItem('offline_card_templates', JSON.stringify(filtered));
  }
}
